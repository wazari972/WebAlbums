...
public class SystemTools {
  ...
  public static ImageUtil getWrapper(String type) {
  ...
  public static void fullscreen(Query query, String type, String id, String page, HttpServletRequest request) {
    ...
    ImageUtil util = getWrapper("image") ;
	...
	if (dir == null) return ;
      ...
      File fPhoto = new File(dir, ""+i+"-p"+currentPage+"-"+enrPhoto.getID()+".jpg") ;
  ...
  public static String shrink(HttpServletRequest request, Photo enrPhoto, int width) {
    ...
    File fPhoto = new File(dir, enrPhoto.getID()+"-"+width+".jpg") ;
    getWrapper(enrPhoto.getType()).shrink(enrPhoto.getImagePath(), fPhoto.toString(), width);
    fPhoto.deleteOnExit() ;
