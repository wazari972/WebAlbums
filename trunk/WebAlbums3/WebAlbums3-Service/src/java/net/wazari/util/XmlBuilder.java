...
public class XmlBuilder {
  ...
  public XmlBuilder add (String name, Object value) {
    ...
    XmlBuilder xml ;
    if (value == null || "".equals(value)) xml = new XmlBuilder(name) ;
    else xml = new XmlBuilder (name, value);
  ...
  private String toString(int indent) {
    if (printed) throw new RuntimeException("Already printed") ;
    else printed = true ;

    String space = indent(indent) ;
    
    if (type == TEXT) return this.text.toString()+"\n" ;
    else if (type == COMMENT) return space + "<!-- " + this.text.toString() + " -->\n" ;
    this.validate() ;

    if (this.exception != null) this.content.add(0, this.exception) ;
	...
	if (attribute == null || "".equals(attribute)) continue ;
