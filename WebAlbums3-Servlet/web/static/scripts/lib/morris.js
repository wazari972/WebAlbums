    ...
    Line.prototype.redraw = function() {
      ...
      for (i = _ref4 = seriesCoords.length - 1; _ref4 <= 0 ? i <= 0 : i >= 0; _ref4 <= 0 ? i++ : i--) {
        ...
        if (coords.length > 1) {
          ...
          this.r.path(path).attr('stroke', this.options.lineColors[i % this.options.lineColors.length]).attr('stroke-width', this.options.lineWidth);
        }
      }
      ...
      for (i = _ref5 = seriesCoords.length - 1; _ref5 <= 0 ? i <= 0 : i >= 0; _ref5 <= 0 ? i++ : i--) {
        ...
        for (_j = 0, _len2 = _ref6.length; _j < _len2; _j++) {
          c = _ref6[_j];
          circle = this.r.circle(c.x, c.y, this.options.pointSize).attr('fill', this.options.lineColors[i % this.options.lineColors.length]).attr('stroke-width', 1).attr('stroke', '#ffffff');
      ...
      for (i = 0, _ref7 = this.series.length - 1; 0 <= _ref7 ? i <= _ref7 : i >= _ref7; 0 <= _ref7 ? i++ : i--) {
        yLabel = this.r.text(0, this.options.hoverFontSize * 1.5 * (i + 1.5) - hoverHeight / 2, '').attr('fill', this.options.lineColors[i % this.options.lineColors.length]).attr('font-size', this.options.hoverFontSize);
        yLabels.push(yLabel);
