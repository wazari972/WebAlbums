...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="get_validate_addr">
    ...
    <xsl:if test="../return_to/name = 'Photos'"><xsl:value-of select="../return_to/name"/>?<xsl:value-of select="$to_add" />&amp;id=<xsl:value-of select="details/photoId/@id" />&amp;page=<xsl:value-of select="../return_to/page" />&amp;album=<xsl:value-of select="../return_to/album" />&amp;albmPage=<xsl:value-of select="../return_to/albmPage" />#<xsl:value-of select="details/photoId/@id" /></xsl:if>
    <xsl:if test="../return_to/name = 'Tags'  "><xsl:value-of select="../return_to/name"/>?<xsl:value-of select="$to_add" />&amp;id=<xsl:value-of select="details/photoId/@id" />&amp;page=<xsl:value-of select="../return_to/page" /><xsl:for-each select="../return_to/tagsAsked">&amp;tagAsked=<xsl:value-of select="." /></xsl:for-each>#<xsl:value-of select="details/photoId/@id" /></xsl:if>
  </xsl:template>
  ...
  <xsl:template match="prev|next|@nexti|@previ|@first|@last">
    <a>
        <xsl:if test="/webAlbums/photos">
          <xsl:attribute name="href">Photos__<xsl:value-of select="../url/album" />_p<xsl:value-of select="." /><xsl:if test="not(/webAlbums/affichage/@static)">_pa<xsl:value-of select="/webAlbums/photos/display/photoList/page/url/albmPage" /></xsl:if>__<xsl:value-of select="/webAlbums/photos/display/album/name" /></xsl:attribute>
	</xsl:if>
