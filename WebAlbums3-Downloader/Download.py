...
def get_choix(themeId, name="", make_index=False):
    global theme
    theme = name
...
def get_a_theme(themeId, name):
    choix = get_choix(themeId, name, make_index=True)
...
def get_index(full=False):
    return get_a_page("index.html", full=full)
...
get_XSLT()
get_index()
get_choix(9, "Grenoble")
get_static()
get_a_tagSet(390, "Jan et Patricia", full=True, make_index=True, get_related_albums=True)
